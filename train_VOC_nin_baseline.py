...
checkpoint_subdir = f'./{checkpoint_dir}/' + dirbase + f'_VOC_moco_partitions_{args.num_partitions}'
...
partitions_file = torch.load('partitions/partitions_hash_mean_PASCAL-VOC_'+str(args.num_partitions)+'.pth')
...
if __name__ == "__main__":
    ...
    for part in range(args.start_partition,args.start_partition+args.num_partition_range):
        ...
        val_dataset = get_voc2007_test(transforms_test)
        idxs=np.arange(len(val_dataset))
        np.random.shuffle(idxs)
        idxs=idxs[:int(1000)]
        val_dataset = torch.utils.data.Subset(val_dataset, idxs)
        trainset = get_voc2007_train(transforms_train)
