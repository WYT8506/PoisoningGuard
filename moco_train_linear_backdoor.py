...
parser.add_argument('--num_partitions', default = 300, type=int, help='number of partitions')
...
def extract_feature(model, train_loader, device):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(train_loader):
            ...
            embeddings_list.append(output.squeeze().unsqueeze(dim=0).to("cpu"))
            if len(target.shape) == 3:
...
def main():
    ...
    print(device)
    print(len(global_train_dataset))
    encoder.to(device)
    ...
    if args.certification == "bagging":
        ...
        for k in range(1,1000):
            ...
            print(k,flush = True)
            seed = (1000*index+k)*int(time.time()) % 123456789
            ...
            optimizer = torch.optim.Adam(params=fc.parameters, lr=lr, weight_decay=args.weightdecay)
            ...
            for epoch in range(args.epoch):
                ...
                if epoch == args.epoch-1:
                    if args.dataset_type == "MS-COCO":
                        ...
                        torch.save(fc.state_dict(), "COCO_moco_models_backdoored_m=%d/save_lr%f_epoch%d_rank%d_seed%d.pth" % (args.m,args.lr, epoch,0,seed))
                    if args.dataset_type == "NUS-WIDE":
                        if not os.path.exists('NUS_moco_models_m='+str(args.m)):
                            os.makedirs('NUS_moco_models_m='+str(args.m))
                        torch.save(fc.state_dict(), "NUS_moco_models_backdoored_m=%d/save_lr%f_epoch%d_rank%d_seed%d.pth" % (args.m,args.lr, epoch,index,seed))
                    if args.dataset_type == "PASCAL-VOC":
                        if not os.path.exists('VOC_moco_models_backoor_m='+str(args.m)):
                            os.makedirs('VOC_moco_models_backdoor_m='+str(args.m))
                        torch.save(fc.state_dict(), "VOC_moco_models_backdoored_m=%d/save_lr%f_epoch%d_rank%d_seed%d.pth" % (args.m,args.lr, epoch,index,seed))
            end = time.time()
            print("time for one base model: ",end-start)
    if args.certification =="dpa":

        torch.backends.cudnn.deterministic = True
        torch.backends.cudnn.benchmark = False

        dirbase = args.dataset_type+'_nin_baseline'
        #if (args.zero_seed):
            #dirbase += '_zero_seed'
        ...
        checkpoint_subdir = f'./{checkpoint_dir}/' + dirbase + f'_{args.dataset_type}_moco_backdoored_partitions_{args.num_partitions}'
        ...
        partitions_file = torch.load('partitions/partitions_hash_mean_'+args.dataset_type+'_'+str(args.num_partitions)+'.pth')
        partitions = partitions_file['idx']
        args.start_partition = 0#(rank%2)*(int(args.num_partitions/2))
        args.num_partition_range = int(args.num_partitions)
        ...
        for part in range(args.start_partition,args.start_partition+args.num_partition_range):
            ...
            part_indices = partitions[part]
            for e in part_indices:
                if e > 82000:
                    e =0
            part_indices = torch.tensor(part_indices)
        ...
        # Training
            for epoch in range(args.epoch):
                train(fc, train_feature_loader, optimizer, None, None,device)
                if epoch%10 == 9:
                    eval(fc, val_feature_loader, 0,device)
                    #eval(model, train_loader, sigma,device)
                    print("DEVICE: ",device, "EPOCH FINISHED: ", epoch,flush=True)
            ...
            print("time for one base model:", end-start)
    if args.certification == "no_defense":
        ...
        parameters = add_weight_decay(fc, args.weightdecay)
        # true wd, filter_bias_and_bn
        optimizer = torch.optim.Adam(params=parameters, lr=lr, weight_decay=0.0001)
        ...
        for epoch in range(args.epoch):
            ...
            if epoch == args.epoch-1:
                if args.dataset_type == "MS-COCO":
                    if not os.path.exists('COCO_moco_models_no_defense'):
                        os.makedirs('COCO_moco_models_no_defense')
                    torch.save(fc.state_dict(), "COCO_moco_models_no_defense/save_lr%f_epoch%d_rank%d_seed%d.pth" % (args.m,args.lr, epoch,0,seed))
                if args.dataset_type == "NUS-WIDE":
                    if not os.path.exists('NUS_moco_models_m='+str(args.m)):
                        os.makedirs('NUS_moco_models_m='+str(args.m))
                    torch.save(fc.state_dict(), "NUS_moco_models_no_defense/save_lr%f_epoch%d_rank%d_seed%d.pth" % (args.m,args.lr, epoch,index,seed))
                if args.dataset_type == "PASCAL-VOC":
                    if not os.path.exists('VOC_moco_models_backdoor_no_defense'):
                        os.makedirs('VOC_moco_models_backdoor_no_defense')
                    torch.save(fc.state_dict(), "VOC_moco_models_no_defense/save_lr%f_epoch%d_rank%d_seed%d.pth" % (args.m,args.lr, epoch,index,seed))
        end = time.time()
        print("time for training model: ",end-start)
