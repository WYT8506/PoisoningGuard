...
parser.add_argument('--certification', default = "dpa", type=str, help='certification')
...
def eval(model, data_loader, sigma,device):
    ...
    print("time for 1000 datapoints:", end-start)
...
def main():
    ...
    idxs=np.arange(len(global_val_dataset))
    np.random.shuffle(idxs)
    idxs=idxs[:int(1000)]
    global_val_dataset = torch.utils.data.Subset(global_val_dataset, idxs)
    ...
    if args.certification =="dpa":
        ...
        checkpoint_subdir = f'./{checkpoint_dir}/' + dirbase + f'_{args.dataset_type}_moco_partitions_{args.num_partitions}'
    ...
    if args.certification == "no_defense":
        ...
        parameters = add_weight_decay(fc, args.weightdecay)
        # true wd, filter_bias_and_bn
        optimizer = torch.optim.Adam(params=parameters, lr=lr, weight_decay=0.00001)
        ...
        for epoch in range(args.epoch):
            ...
            if epoch == args.epoch-1:
                ...
                if args.dataset_type == "PASCAL-VOC":
                    if not os.path.exists('VOC_moco_models_no_defense'):
                        os.makedirs('VOC_moco_models_no_defense')
