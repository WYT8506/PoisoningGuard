...
parser.add_argument("--certification", type=str, default="bagging")
parser.add_argument("--begin", type=float, default=0)
parser.add_argument("--end", type=float, default=2)
parser.add_argument("--T", type=int, default=100)
parser.add_argument("--num_partitions", type=int, default=260)
...
def main():
    ...
    if args.dataset_type == "PASCAL-VOC":
        ...
        if args.certification == "dpa":
            path = './checkpoints/VOC_nin_baseline_VOC_moco_partitions_'+str(args.num_partitions)
    ...
    if args.dataset_type == "PASCAL-VOC":
        if args.certification == "bagging":
            if not os.path.exists('voc_moco_results_m='+str(args.m)):
                os.makedirs('voc_moco_results_m='+str(args.m))
            np.save('voc_moco_results_m='+str(args.m)+'/all_outputs.npy', all_outputs)
            np.save('voc_moco_results_m='+str(args.m)+'/all_targets.npy', all_targets)
        if args.certification =="dpa":
            if not os.path.exists('./voc_moco_results_partitions='+str(args.num_partitions)):
                os.makedirs('./voc_moco_results_partitions='+str(args.num_partitions))
            np.save('voc_moco_results_partitions='+str(args.num_partitions)+'/all_outputs.npy', all_outputs)
            np.save('voc_moco_results_partitions='+str(args.num_partitions)+'/all_targets.npy', all_targets)
    if args.dataset_type == "NUS-WIDE":
        if args.certification == "bagging":
