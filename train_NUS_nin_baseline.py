...
parser.add_argument('--num_partitions', default = 260, type=int, help='number of partitions')
...
checkpoint_subdir = f'./{checkpoint_dir}/' + dirbase + f'_NUS_moco_partitions_{args.num_partitions}'
...
partitions_file = torch.load('partitions_hash_mean_NUS-WIDE_'+str(args.num_partitions)+'.pth')
...
if __name__ == "__main__":
    ...
    for part in range(args.start_partition,args.start_partition+args.num_partition_range):
        ...
        val_dataset = nuswide.NUSWIDEClassification('.', 'test',transform = transforms_test)
        ...
        trainset = nuswide.NUSWIDEClassification('.', 'trainval',transform = transforms_train)
