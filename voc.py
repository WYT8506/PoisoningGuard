...
import pickle
from helper_functions import Warp
...
def download_voc2007(root):
    ...
    if not os.path.exists(path_devkit):
        ...
        if not os.path.exists(cached_file):
            ...
            urllib.request.urlretrieve(urls['devkit'], cached_file)
        # extract file
    ...
    if not os.path.exists(path_images):
        ...
        if not os.path.exists(cached_file):
            ...
            urllib.request.urlretrieve(urls['trainval_2007'], cached_file)
        # extract file
        print('[dataset] Extracting tar file {file} to {path}'.format(file=cached_file, path=root))
        cwd = os.getcwd()
        tar = tarfile.open(cached_file, "r")
    ...
    if not os.path.exists(test_anno):
        ...
        if not os.path.exists(cached_file):
            ...
            urllib.request.urlretrieve(urls['test_images_2007'], cached_file)
    ...
    if not os.path.exists(test_image):
        ...
        if not os.path.exists(cached_file):
            ...
            urllib.request.urlretrieve(urls['test_anno_2007'], cached_file)
...
def get_voc2007_train(transform):
    return Voc2007Classification("voc2007","trainval",transform)
def get_voc2007_test(transform):
    return Voc2007Classification("voc2007","test",transform)
